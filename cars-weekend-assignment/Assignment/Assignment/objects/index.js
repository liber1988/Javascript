...
const CarAgencyManager = {
  ...
  searchAgency: function (idOrName) {
    for (const element of this.agencies) {
      if (
        element["agencyName"] === idOrName ||
        element["agencyId"] === idOrName
      ) {
        return element;
      }
    }
  ...
  getAllAgencies: function () {
    return this.agencies.map((x) => x["agencyName"]);
  },
  ...
  addCarToAgency: function (agencyId, car) {
    for (let element of this.agencies) {
      if (element.agencyId === agencyId) {
        element.cars.push(car);
        return true;
      }
    }
    return false;
  ...
  mergers: function (element) {
    for (let indexCar = 0; indexCar < element.cars.length; indexCar++) {
      for (let j = indexCar + 1; j < element.cars.length; j++) {
        if (element.cars[indexCar].brand === element.cars[j].brand) {
          element.cars[indexCar].models.push(...element.cars[j].models);
          element.cars.splice(j, 1);
        }
      }
    }
  },
  carObjectViaCarNumberAndAgency: function (carId) {
    for (
      let egencyIndex = 0;
      egencyIndex < this.agencies.length;
      egencyIndex++
    ) {
      for (
        let carsIndex = 0;
        carsIndex < this.agencies[egencyIndex].cars.length;
        carsIndex++
      ) {
        for (
          let carNumberIndex = 0;
          carNumberIndex <
          this.agencies[egencyIndex].cars[carsIndex].models.length;
          carNumberIndex++
        ) {
          if (
            this.agencies[egencyIndex].cars[carsIndex].models[carNumberIndex]
              .carNumber === carId
          ) {
            const obj = {
              brand: this.agencies[egencyIndex].cars[carsIndex].brand,
              models: [
                this.agencies[egencyIndex].cars[carsIndex].models[
                  carNumberIndex
                ],
              ],
            };
  ...
  changeAgencyCashOrCredit: function (agencyId, cashOrCredit) {
    ...
    if (agencyIndex != -1) {
      agencies[agencyIndex].cash = cashOrCredit;
      return true;
    } else {
      return false;
    }
  ...
  getTotalAgencyRevenue: function (agencyId) {
    {
      ...
      if (agencyIndex !== -1) {
        ...
        const carPrices = agency.cars.map((car) =>
          car.models.reduce((total, model) => total + model.price, 0)
        );
        return carPrices.reduce((total, price) => total + price, 0);
      } else {
        return "Wring egency ID";
      }
    }
  },
  ...
  transferCarBetweenAgencies: function (fromAgencyId, toAgencyId, carId) {
    const carobject = this.carObjectViaCarNumberAndAgency(carId);
    this.addCarToAgency(toAgencyId, carobject);
    this.removeCarFromAgency(fromAgencyId, carId);
  },
};
CarAgencyManager.transferCarBetweenAgencies("Plyq5M5AZ", "26_IPfHU1", "S6DL1");
console.log(CarAgencyManager.agencies[1].cars[0].models[3]);
